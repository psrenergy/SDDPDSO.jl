function export_results(x, n, d, par, sims, m)
    ...
    export_3D_Matrix_as_graf(x,D_Matrix, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_original_demand", d.bus_name, UNIT ="MWh")
    ...
    if n.cir > 0
        ...
        export_as_graf(x, sims, :flw, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_cirflw", d.cir_name, UNIT ="MWh")

        par.flag_verbose && CSV.write(
            joinpath(x.PATH,"results","usecir.csv"),
            export_result_usecir(sims,d.cir_capacity,d.cir_name)
        )
        export_result_usecir_as_graf(x, n, sims, d.cir_capacity, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_cir_use", d.cir_name, UNIT ="%")
    ...
    if n.ther > 0
        ...
        export_as_graf(x, sims, :gen_die, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_thermal_generation", d.ter_name, UNIT ="MWh")
    ...
    if n.gnd > 0
        ...
        export_as_graf(x, sims, :gen_sol, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_renewable_generation", d.gnd_name, UNIT ="MWh")

        par.flag_verbose && CSV.write(
            joinpath(x.PATH,"results","renewable_curtailment.csv"),
            simulate_create_result_dif_table(sims,:gen_sol_max,:gen_sol,d.gnd_name)
        )
        export_dif_as_graf(x, sims, :gen_sol_max,:gen_sol, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_renewable_curtailment", d.gnd_name, UNIT ="MWh")
    ...
    export_as_graf(x, sims, :def, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_load_deficit", ["def$i" for i in 1:n.load], UNIT ="MWh")

    par.flag_verbose && CSV.write(
        joinpath(x.PATH,"results","bus_curtailment.csv"),
        simulate_create_result_table(sims,:cur,["cur$i" for i in 1:n.bus])
    )
    export_as_graf(x, sims, :cur, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_bus_curtailment", ["cur$i" for i in 1:n.bus], UNIT ="MWh")
    ...
    if n.bat > 0 && (par.flag_bat)
        ...
        export_as_graf(x, sims, :bat_d, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_battery_discharge", d.bat_name, UNIT ="MWh")

        par.flag_verbose && CSV.write(
            joinpath(x.PATH,"results","battery_charge.csv"),
            simulate_create_result_table(sims,:bat_c,d.bat_name)
        )
        export_as_graf(x, sims, :bat_c, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_battery_charge", d.bat_name, UNIT ="MWh")
    ...
    if par.flag_dem_rsp
        ...
        export_3D_Matrix_as_graf(x,Upper_DR, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_dr_upper_bound", d.bus_name, UNIT ="MWh")
    
        Lower_DR = TransformaDemandaMatriz_LowerRD(par.bus_map_dem,par.demand,n.bus,par.stages,par.set_dem_rsp,par.dem_rsp_shift)
        export_3D_Matrix_as_graf(x,Lower_DR, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_dr_lower_bound", d.bus_name, UNIT ="MWh")
        ...
        export_StateVar_as_graf(x, sims, :total_load, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_dr_accumulated_load", d.load_name[par.set_dem_rsp], UNIT ="MWh")

        par.flag_verbose && CSV.write(
            joinpath(x.PATH,"results","dr_load.csv"),
            simulate_create_result_table(sims,:dr,d.load_name[par.set_dem_rsp])
        )
        export_as_graf_convertingArray(x, sims, :dr, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_dr_load", d.load_name[par.set_dem_rsp], UNIT ="MWh")
        
        par.flag_verbose && CSV.write(
            joinpath(x.PATH,"results","dr_deficit.csv"),
            simulate_create_result_table(sims,:dr_def,d.load_name[par.set_dem_rsp])
        )
        export_as_graf_convertingArray(x, sims, :dr_def, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_dr_deficit", d.load_name[par.set_dem_rsp], UNIT ="MWh")

        par.flag_verbose && CSV.write(
            joinpath(x.PATH,"results","dr_curtailment.csv"),
            simulate_create_result_table(sims,:dr_cur,d.load_name[par.set_dem_rsp])
        )
        export_as_graf_convertingArray(x, sims, :dr_cur, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_dr_curtailment", d.load_name[par.set_dem_rsp], UNIT ="MWh")
    end
    ...
    export_weighted_shadow_price_as_graf(x, sims, D_Matrix, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_bus_marginal_cost", ["Bus Marginal Cost"], UNIT ="\$")
    ...
    if par.flag_import
        ...
        export_as_graf(x, sims, :imp, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_energy_import", d.bus_name, UNIT ="MWh")
    ...
    if par.flag_export
        ...
        export_as_graf(x, sims, :exp, joinpath(x.PATH,"results"),CSV = par.flag_CSV, "DSO_energy_export", d.bus_name, UNIT ="MWh")
        par.flag_verbose && CSV.write(
